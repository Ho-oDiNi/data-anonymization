...
import ru.data.anonymization.tool.methods.options.type.SelectionMethod;
...
public class SelectionMethodView {
    ...
    public void configView(
            ...
            VBox vBox) throws
            IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(ValueVarianceView.class.getResource(
                "selection-method-view.fxml"));
        ...
        variance.setTextFormatter(new TextFormatter<>(
    ...
    private void saveAction(String table, String column, ShowMode mode) {
        if (customName.getText().isEmpty() || customName.getText().isBlank()) {
            ...
            DialogBuilder.createErrorDialog("Нужно ввести процент выборки!");
        } else {
            SelectionMethod dto;
            if (depersonalizationService.isContainsKey(customName.getText())) {
                dto = (SelectionMethod) depersonalizationService.getMethod(customName.getText());
                dto.setPercent(Integer.parseInt(variance.getText()));
            } else {
                dto = new SelectionMethod();
