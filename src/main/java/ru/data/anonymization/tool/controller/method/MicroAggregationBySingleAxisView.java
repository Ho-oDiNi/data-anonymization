...
import ru.data.anonymization.tool.methods.options.type.MicroAggregationBySingleAxis;
...
public class MicroAggregationBySingleAxisView {
    ...
    @FXML
    private ComboBox<String> axisColumn;
    @FXML
    ...
    public void configView(String title, String table, ShowMode mode, String name, VBox vBox) throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(MicroAggregationBySingleAxisView.class.getResource("micro-aggregation-by-single-axis-view.fxml"));
        stage = ComponentUtils.modalStageView(fxmlLoader, "Настройка метода (" + title + ")", "gears.png");
        tableName.setText(table);

        ObservableList<String> list = FXCollections.observableArrayList();
        axisColumn.setItems(list);
        ...
        if (mode.equals(ShowMode.EDIT)) {
            ...
            MicroAggregationBySingleAxis dto = (MicroAggregationBySingleAxis) depersonalizationService.getMethod(name);

            List<String> selectedElement = dto.getColumn();
            columnList.removeAll(selectedElement);

            axisColumn.getItems().add(dto.getAxisColumn());
            List<String> axis = new ArrayList<>(selectedElement);
            axis.remove(dto.getAxisColumn());
            axisColumn.getItems().addAll(axis);
            axisColumn.getSelectionModel().select(0);
    ...
    private void moveToRight() {
        ...
        if (selectedItem != null) {
            ...
            axisColumn.getItems().add(selectedItem);
        }
    }
    ...
    private void moveToLeft() {
        ...
        if (selectedItem != null) {
            ...
            axisColumn.getItems().remove(selectedItem);
        }
    }
    ...
    private void saveAction(String table, ShowMode mode) {
        ...
        String axis = axisColumn.getSelectionModel().getSelectedItem();
        int count = 0;
        ...
        if (customName.getText().isEmpty() || customName.getText().isBlank()) {
            ...
            DialogBuilder.createErrorDialog("Нужно выбрать главный атрибут!");
        } else if (count < 1) {
            DialogBuilder.createErrorDialog("Параметр k должен быть больше 0!");
        } else {
            MicroAggregationBySingleAxis dto = new MicroAggregationBySingleAxis();
            dto.setNameTable(table);
            dto.setNamesColumn(columns);
            dto.setAxisColumn(axis);
            dto.setK(count);
