...
# cat payload.json | python tvae.py --n 500 --target total
# Get-Content payload.json | python tvae.py --n 500 --target total
...
def synthesize_with_tvae(
    df: pd.DataFrame,
    n_rows: int = 0,
    target_column: Optional[str] = None,
) -> pd.DataFrame:
    """
    Generates synthetic data using SynthCity TVAE plugin.
...
def main() -> None:
    ...
    try:
        syn_df = synthesize_with_tvae(df, n_rows=n_rows, target_column=target_col)
