...
# cat payload.json | python great.py --n 500 --target total
# Get-Content payload.json | python great.py --n 500 --target total
...
def _extract_plugin_args(config: dict, key: str) -> dict:
    """Поддерживаем config.plugin_args и config.<key> (например great_args)."""
...
def synthesize_with_great(
    ...
    """
    Генерирует синтетические данные с помощью SynthCity GReaT (plugin id: 'great').

    Requirements:
      pip install "synthcity[all]" pandas
      (может потребоваться установка зависимостей, которые подтянет synthcity для GReaT)
    ...
    - Additional plugin kwargs can be passed via config.plugin_args or config.great_args.
    """
    ...
    plugin = Plugins().get("great", **kwargs)
...
def main() -> None:
    ...
    if config:
        ...
        plugin_args = _extract_plugin_args(config, "great_args")

    try:
        syn_df, warnings = synthesize_with_great(
