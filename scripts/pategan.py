...
# cat payload.json | python pategan.py --n 500 --target total
# Get-Content payload.json | python pategan.py --n 500 --target total
...
def synthesize_with_pategan(
    df: pd.DataFrame,
    n_rows: int = 0,
    target_column: Optional[str] = None,
) -> pd.DataFrame:
    """
    Generates synthetic data using SynthCity PATEGAN plugin.
...
def main() -> None:
    ...
    try:
        syn_df = synthesize_with_pategan(df, n_rows=n_rows, target_column=target_col)
